...
--[[ 
 ...
 - Made module-less by Edward Garcia @ 2011
...
local director = {}
local loaded = {} -- Will use this table in lieu of package.loaded
...
director.directorView = display.newGroup()
local directorView = director.directorView
--
...
local unloadScene = function( moduleName )
	...
	if moduleName ~= "main" then
		...
		if type( loaded[ moduleName ] ) ~= "nil" then
			
			------------------
			-- Search for the clean() function
			------------------
			
			if loaded[ moduleName ].clean then
				
				------------------
				-- Execute
				------------------
				
				local handler, message = pcall( loaded[ moduleName ].clean )
				--
				if not handler then
...
local loadScene = function( moduleName, target, params )
	...
	if not loaded[ moduleName ].new then
		showError( "Module '" .. tostring( moduleName ) .. "' must have a new() function." )
		return false
	end
	--
	local functionName = loaded[ moduleName ].new
...
function director:openPopUp( params, newPopUpScene, onClose )
	...
	if not loaded[ newPopUpScene ].new then
	...
	local functionName = loaded[ newPopUpScene ].new
	--
